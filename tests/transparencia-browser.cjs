...
(async()=>{
 ...
 await cmd('Page.navigate',{url:base+'/descargar-serie.html?sorteo=token&serie=1'});
 await wait('!!document.querySelector(".consulta-dni")');
 assert.equal(await evaluate('document.querySelector(".consulta-dni input").value'),'');
 await evaluate('document.querySelector(".consulta-dni input").value="12345678";document.querySelector(".consulta-dni").requestSubmit()');
 await wait('document.querySelectorAll(".series-dni .resultado-canto").length === 2');
 assert.deepEqual(consultaDni,{dni:'12345678'});
 assert.equal(await evaluate('document.querySelectorAll("[data-pdf]").length'),3);
 assert.match(await evaluate('document.querySelector(".mensaje-dni").textContent'),/Pendientes de pago: series 3/);
 assert.equal(await evaluate('location.href.includes("12345678")'),false);
 assert.equal(await evaluate('document.documentElement.scrollWidth <= window.innerWidth'),true);
 await evaluate('document.querySelector(".consulta-dni input").value="11111111";document.querySelector(".consulta-dni").requestSubmit()');
 await wait('document.querySelector(".mensaje-dni").textContent.includes("No encontramos")');
 assert.equal(await evaluate('document.querySelectorAll(".series-dni .resultado-canto").length'),0);
