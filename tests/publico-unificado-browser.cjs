...
app.post('/vivo/:token/mis-series',(q,r)=>r.json({series:q.body.dni === '12345678' ? [1,2].map(numeroSerie=>({numeroSerie,cartones:[testCard],marcas:{'card-2':[1]}})) : []}));
app.get('/vivo/:token',(_q,r)=>r.set('Cache-Control','no-store').json({type:'STATE_SNAPSHOT',...state}));
let eventoPublico = null;
app.get('/s/publico',(_q,r)=>r.json({tipo:'BINGO',titulo:'Sorteo unificado',estado:'ACTIVO',tablero:[{numero:1,estado:'LIBRE'}],enVivo:eventoPublico,presentacion:{inicioProgramado:new Date(Date.now()+3600000).toISOString(),imagenesPremios:['data:image/png;base64,iVBORw0KGgo='],descripcionesPremios:['Primer premio: bicicleta <especial>']}}));
let descargaSolicitada = null;
app.post('/s/publico/mis-series', (q,r) => r.json({series:q.body.dni === '12345678' ? [1,2].map(numeroSerie => ({numeroSerie,nombre:'Titular',cartones:[testCard]})) : [],pendientes:[]}));
app.post('/s/publico/mis-series/descargar', (q,r) => {descargaSolicitada=q.body;r.type('application/pdf').send('%PDF-1.4');});
...
(async()=>{
 ...
 assert.equal(await evaluate('document.querySelector("#premios figcaption").textContent'),'Primer premio: bicicleta <especial>');
 ...
 await wait('document.querySelectorAll(".series-dni section").length === 2');
 assert.equal(await evaluate('document.querySelectorAll(".series-dni [data-pdf]").length'),1);
 assert.equal(await evaluate('document.querySelectorAll(".series-dni .resultado-canto").length'),2);
 await evaluate(`document.querySelector('.series-dni [data-pdf]').click()`);
 await wait('!document.querySelector(".series-dni [data-pdf]").disabled');
 assert.deepEqual(descargaSolicitada,{dni:'12345678'});
 await evaluate(`const campo=document.querySelector('.consulta-dni input');campo.value='87654321';campo.dispatchEvent(new Event('input',{bubbles:true}));document.querySelector('.consulta-dni').requestSubmit()`);
 await wait('document.querySelector(".mensaje-dni").textContent.includes("No encontramos")');
 assert.equal(await evaluate('document.querySelectorAll(".series-dni [data-pdf]").length'),0);
 ...
 await evaluate(`var doc=document.querySelector('iframe').contentDocument;doc.getElementById('dniReingreso').value='12345678';doc.getElementById('btnCargarSerieEspectador').click()`);
